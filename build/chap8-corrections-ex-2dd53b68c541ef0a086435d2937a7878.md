...
* chapitre {ref}`chap-APetP`
    ...
    1. $\left| \alpha \right|^{2} = 5$ n'est pas une probabilité. Cette dernière doit être comprise entre $0$ et $1$.
    ...
    1. $\left| \alpha^{*} \beta \right| = 26$. L'angle de référence est $\arctan(5/1)$ et l'argument est dans le troisième quadrant ($\Re (\alpha^{*} \beta)= -1$ et $\Im (\alpha^{*} \beta)= -5$) donc $\arg(\alpha^{*} \beta) = \pi +\arctan(5/1)$.
    :::
...
* chapitre {ref}`chap-evolutionT`
    :::{solution} exo-hamilt-qubit
    :label: solexo-hamilt-qubit
    :class: dropdown

    1. $\widehat{H}_{\left\{ \left| 0 \right\rangle, \left| 1 \right\rangle \right\}}=\begin{pmatrix}
    0 & 0 \\
    0 & \hbar \omega
    \end{pmatrix}$

    1. Les valeurs propres sont $0$ et $\hbar \omega$ respectivement associées aux vecteurs propres :
    $\left| 0 \right\rangle_{\left\{ \left| 0 \right\rangle, \left| 1 \right\rangle \right\}}= $ et  $\left| 1 \right\rangle = \begin{pmatrix}
    1 \\
    0 
    \end{pmatrix}$
    ...
    $\left| 1 \right\rangle_{\left\{ \left| 0 \right\rangle, \left| 1 \right\rangle \right\}}= $ et  $\left| 1 \right\rangle = \begin{pmatrix}
    0\\
    1
    \end{pmatrix}$

    1. $\left| \Psi(t) \right\rangle = \alpha e^{0} \left| 1 \right\rangle + \beta e^{-i \omega t} \left| 1 \right\rangle$
