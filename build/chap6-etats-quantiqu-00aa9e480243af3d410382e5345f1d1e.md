...
La norme $\left\| \left.  \left| \Psi \right.  \right\rangle \right\|$ d'un vecteur $\left| \Psi \right\rangle$ de $\varepsilon$ est
définie :
...
$$\left( \left| \left.  \Phi \right\rangle,\lambda_{1}\left.  \left| \Psi_{1} \right.  \right\rangle + \lambda_{2}\left.  \left| \Psi_{2} \right.  \right\rangle \right.  \right) =\\
\lambda_{1}\left( \left| \left.  \Phi \right\rangle,\left.  \left| \Psi_{1} \right.  \right\rangle \right.  \right) + \lambda_{2}\left( \left| \left.  \Phi \right\rangle,\left.  \left| \Psi_{2} \right.  \right\rangle \right.  \right) =
\lambda_{1}\left\langle \Phi \middle| \Psi_{1} \right\rangle + \lambda_{2}\left\langle \Phi \middle| \Psi_{2} \right\rangle$$

- anti-linéarité par rapport au premier ket :

$$\left( \lambda_{1}\left| \Phi_{1} \right\rangle + \lambda_{2}\left| \Phi_{2} \right\rangle,\left| \Psi \right\rangle \right) =
{\lambda_{1}}^{*}\left( \left| \Phi_{1} \right\rangle,\left| \Psi \right\rangle \right) + {\lambda_{2}}^{*}\left( \left| \Phi_{2} \right\rangle,\left| \Psi \right\rangle \right) =\\
{\lambda_{1}}^{*}\left\langle \Phi_{1} \middle| \Psi \right\rangle + {\lambda_{2}}^{*}\left\langle \Phi_{2} \middle| \Psi \right\rangle = \left( {\lambda_{1}}^{*}\left\langle \Phi_{1} \right| + {\lambda_{2}}^{*}\left\langle \Phi_{2} \right| \right)\left| \Psi \right\rangle$$
...
$\left| \Psi \right\rangle$ $\in$ $\varepsilon$. La correspondance ket 	$\rightsquigarrow$
bra appelée **conjugaison hermitique** est anti-linéaire.

:::{attention}
On mémorisera donc que :

$\lambda_{1}\left| \Phi_{1} \right\rangle + \lambda_{2}\left| \Phi_{2} \right\rangle\;\rightsquigarrow \;{\lambda_{1}}^{*}\left\langle \Phi_{1} \right| + {\lambda_{2}}^{*}\left\langle \Phi_{2} \right|$
:::
...
:::{attention}
En général, $\widehat{A}\widehat{B}$ est différent de $\widehat{B}\widehat{A}$ sauf dans des cas particuliers que nous nous attacherons à décrire dans la suite.
:::
...
```{math}
:label: eq-prodAdjoints

\left\langle w \right| = \left\langle u \right|{\widehat{A}}^{\dagger}{\widehat{B}}^{\dagger}
```

soit en identifiant {eq}`eq-adjoint` et {eq}`eq-prodAdjoints` :
...
complexe de son transposé. L'élément diagonal 
$\left\langle X \right|\widehat{A}\left| X \right\rangle$ est donc réel si $\widehat{A}$ est hermitique.
```
...
:::{exercise}
:class: dropdown
:label: exo-repr-adjoint

En utilisant {eq}`eq-adjointOp`, calculer l'adjoint des opérateurs suivants :

1. ${\widehat{A}}_{\mathcal{B}} = \begin{pmatrix}
2 & -i & 0 \\
i & 0 & -3 \\
0 & 3 & -1
\end{pmatrix}$

1. ${\widehat{B}}_{\mathcal{B}} = \begin{pmatrix}
2 & -i & 0 \\
i & 0 & 3 \\
0 & 3 & -1
\end{pmatrix}$

1. ${\widehat{C}}_{\mathcal{B}} = \begin{pmatrix}
2 & -i & 0 \\
i & a + ib & 3i \\
0 & -3i & -1
\end{pmatrix}\; $ où $a, b \in \mathbb{R}$

1. ${\widehat{D}}_{\mathcal{B}} = \begin{pmatrix}
2 & 2 & 0 \\
2 & a & 3i \\
0 & -3i & -1
\end{pmatrix}\; $ où $a \in \mathbb{R}$

1. ${\widehat{E}}_{\mathcal{B}} = \begin{pmatrix}
b & 2 & 0 \\
2 & a & 3i \\
0 & -3i & -1
...
base. C'est pourquoi ces coefficients sont appelés <u>{index}`amplitude d'états`</u>.
...
La projection du vecteur d'état $\left| \Psi \right\rangle$ sur un vecteur de base
...
:::{prf:definition}
:label: my-fermeture
$$\sum_{k = 1}^{N}{\left| u_{k} \right\rangle\left\langle u_{k} \right|} = \widehat{1}$$

Ainsi les vecteurs d'une base orthonormée vérifient la relation de fermeture (complétude) et l'orthonormalité (relation {eq}`eq-orthonormal`).
:::

:::{attention}
La relation {eq}`eq-orthonormal` ne signifie pas que les vecteurs $\left| u_{k} \right\rangle$ forment une base othornormée car la relation ne fait état de la {index}`complétude` de l'ensemble.
:::

:::{exercise}
:class: dropdown
:label: exo-norma
Le développement de $\left| \Phi \right\rangle$ sur une base
...
:::{note}
Si l'ensemble $\left\{ \left| u_{k} \right\rangle \right\}$
forme une base orthonormée de l'espace des états d'un système,  $\left\{ \left\langle \Psi u_{k} \right| \right\}$ forme une base pour l'espace dual $\varepsilon^{\dagger}$.
:::
...
:::{prf:definition}
:label: my-bra
Par application de la relation de fermeture, on représente le bra $\left\langle \Psi \right|$ dans la base $\left\{ \left\langle \Psi u_{k} \right| \right\}$ sous la forme d'une matrice ligne :
...
:::{exercise}
:class: dropdown
:label: exo-prodmat-adjoint

Soient les représentations matricielles de $\widehat{E}$  et $\left| \phi \right\rangle$ dans la base $\mathcal{B}$ :

${\widehat{E}}_{\mathcal{B}} = \begin{pmatrix}
b & 2 & 0 \\
2 & a & 3i \\
0 & -3i & -1
\end{pmatrix}\;$ où $a, b \in \mathbb{C}$

${\left| \phi \right\rangle}_{\mathcal{B}}=\begin{pmatrix}
\alpha \\
\beta \\
\gamma
\end{pmatrix}$

1. Calculer la représentation matricielle dans la base $\mathcal{B}$ du ket $\widehat{E} \left| \phi \right\rangle$.

1. Calculer la représentation matricielle du conjugué hermitique du ket $\widehat{E} \left| \phi \right\rangle$.

1. Calculer la représentation matricielle du bra $\left\langle \phi \right|\mathcal{B}^{\dagger}$.
...
${\widehat{A}}_{\left\{ \left| e_{i} \right\rangle \right\}} = \begin{pmatrix}
2 & 1 & 0 \\
1 & 3 & 1 \\
0 & 1 & 2
\end{pmatrix}\;$ où $a, b \in \mathbb{C}$

1. Vérifier que $\widehat{A}$ est hermitique.

1. Rappeler les deux propriétés d'un opérateur hermitique.

1. Calculer le polynôme caractéristique $\mathcal{det}\left( \widehat{A} - \lambda \widehat{I} \right)$

1. En déduire les valeurs propres $\lambda_i$ de $\widehat{A}$ classées par valeurs décroissantes.

1. Pour chaque valeur propre $\lambda_i$, déterminer le vecteur propre associé par résolution de $\widehat{A} v_i =\lambda_i v_i$. On normalisera à l'unité chaque vecteur propre.

1. A partir des représentations matricielles des vecteurs propres dans la base $\left\{ \left| e_{i} \right\rangle \right\}$, montrer matriciellement qu'ils vérifient la relation de fermeture.

1. Donner la représentation matricielle de l'observable $\widehat{A}$ dans la base de ses états propres  $\left\{ \left| v_{i} \right\rangle \right\}$.

1. La {index}`décomposition spectrale` de $\widehat{A}$ s'écrit : $\widehat{A} = \sum_{i=1}^{3} \lambda_i \left| v_{i} \right\rangle \left\langle v_{i} \right|$. Montrer que l'action de cette décomposition sur chaque vecteur propre est conforme à la notion de vecteur propre.

1. Donner la représentation matricielle de $\widehat{A} = \sum_{i=1}^{3} \lambda_i \left| v_{i} \right\rangle \left\langle v_{i} \right|$ dans la base des états $\left\{ \left| e_{i} \right\rangle \right\}$. Conclusion.

1. Soit $\widehat{S}$ la matrice unitaire formée par les vecteurs propres colonne $v_1$, $v_2$ et $v_3$ et $\widehat{S}^{T}$ sa transposée. Calculer le produit matricielle $\widehat{S}^{T}\widehat{A}\widehat{S}$.
:::

```{code-cell} ipython3
:label: markdown-lin-alg
import numpy as np
...
la variable. On définit la {index}`densité de probabilité` $g(x,t)$ à
...
Lorsque $C$ est finie, $\Psi(x,t)$ est  une {index}`fonction d'onde de carré sommable`. On peut
...
d'obtenir comme résultat une valeur propre $a_n$ donnée **<u>non
