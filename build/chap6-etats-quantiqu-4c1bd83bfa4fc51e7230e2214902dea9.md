...
Soit l'observable $\widehatA$ :

${\widehatA}_ $\left\{ \left| e_{i} \right\rangle \right\}$ = \begin{pmatrix}
2 & 1 & 0 \\
1 & 3 & 1 \\
0 & 1 & 2
\end{pmatrix}\;$ où $a, b \in \mathbb{C}$

1. Vérifier que $\widehatA$ est hermitique.

1. Rappler les deux propriétés d'un opérateur hermitique.

1. Calculer le polynôme caractéristique $mathcal{det}\left( \widehatA - \lambda \widehatI \right)$

1. En déduire les valeurs propres de $\widehatA$.

1. Pour chaque valeur propre $\lamda_i$, déterminer le vecteur propre associé par résolution de $\widehatA v_i =\lamda_i v_i$. On normalisera à l'unité chaque vecteur propre.
...
1. La décomposition spectrale de $\widehatA$ s'écrit : $\widehatA = \sum_{i=1}^{3} \lamda_i \left| v_{i} \right\rangle \left\langle v_{i} \middle|$. Montrer que l'action de cette décomposition sur chaque vecteur propre est conforme à la notion de vecteur propre.

1. Ecrire $\widehatA = \sum_{i=1}^{3} \lamda_i \left| v_{i} \right\rangle \left\langle v_{i} \middle|$ dans la base des états $\left\{ \left| e_{i} \right\rangle \right\}$. Conclusion.
:::
...
1. $mathcal{det}\left( \widehatA - \lambda \widehatI \right)=\left( 2-\lambda\right)\left(\lambda^2-5\lambda+4\right)=\left( 2-\lambda\right)\left(\lambda-1\right)\left(\lambda-4\right)$
