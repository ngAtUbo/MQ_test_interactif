...
Nous postulons donc que l\'**état quantique** {index}`Etat quantique` d\'un système physique quelconque est
caractérisé par un vecteur d\'état appartenant à un espace vectoriel
linéaire complexe appelé **espace des états du système** {index}`Espace des états`.
...
Un élément quelconque de $\varepsilon{*}$ est appelé **vecteur
...
Par définition un opérateur est dit hermitique {index}`Hermitique` (ou hermitien ou encore auto-adjoint) s\'il est égal
...
superposition linéaire d\'états {index}`Superposition linéaire d'états`.
...
et $\overrightarrow{k}$ vérifient la <u>relation de
...
un opérateur hermitique dont les kets propres forment une base de
l\'espace des états. Ce type d\'opérateur joue un rôle capital en
théorie quantique.
...
fonction d'onde {index}`Fonction d’onde`
...
une région finie de l'espace. Elle se trouve dans un état
