...
```{note}
Les chemins ayant une longueur différentes, la phase accumulée $\Phi$ n'est pas la même. Il existe donc un déphasage entre les amplitudes associées à chaque chemin.
````
...
**Estimons la plus petite valeur de $\theta$ compatible avec cette condition d'amplutude résultat nulle dans la direction $\theta$.**
