---
title: "Probabilités et amplitudes quantiques"
description: "Amplitudes de probabilité, probabilités et principes."
authors:
...
certains phénomènes impliquant des ondes électromagnétiques.
...
situations faisant intervenir un grand nombre de photons.
:::
...
**d'{index}`amplitude de probabilité`** est nécessaire pour expliquer
...
Cette relation est appelée loi de {cite}`malus1809propriete`. 
...
1. une proportion $\cos^{2}\theta$ des photons est transmise
intégralement.
...
lumière à l'interface entre deux milieux transparents [{cite}`rayleigh1879reflection`]. C'est un
phénomène qui est très bien expliqué par la théorie électromagnétique de
Maxwell. Dans le cas d'une incidence normale, le\
l'expérience montre, comme le rappelle Fresnel dans son mémoire [{cite}`fresnel2016lumiere`], que 4 % de l'énergie incidente est réfléchie ; il
...
:width: 90%
Clichés pris avec : a) $3000$ photons b) $1,2 \times 10^4$ photons c) $9,3 \times 10^4$ photons d) $7,6 \times 10^5$ photons e) $3,6 \times 10^6$ photons f) $2,8 \times 10^7$ photons
```

L'image est tirée de l'ouvrage de {cite}`french2018introduction` d'après des clichés de {cite}`rose1953quantum`.
...
lorsque les deux trous sont ouverts est la suivante :
...
En effet, si c'était le cas on pourrait dire que
le nombre d'électrons reçus en chaque point est égal au nombre
d'électrons ayant traversé le trou 1 auquel on ajoute le nombre d'électrons ayant traversé le trou 2 :

 ```{attention}
 Le nombre total d'électrons reçus en tout point lorsque
les deux trous sont ouverts ne pourrait jamais être inférieur au nombre
reçus au même point lorsqu'un seul trou est
ouvert. Il n'y aurait donc pas d'interférence.
````
...
```{figure} media/eclairFentes2.png
...
d'**{index}`amplitude de probabilité de transition`** :

:::{tip} Amplitude de probabilité de transition
:label: my-AP
Une amplitude de probabilité de transition (AP) est un nombre complexe dont le carré du module représente la probabilité de
cette transition.
:::

Une telle amplitude est
un nombre complexe dont le carré du module représente la probabilité de
cette transition.

En présence de deux ou plusieurs voies ou
...
**superposition des amplitudes** ou  {index}`principe de superposition des amplitudes` (PSAP) joue un rôle capital
en théorie quantique :

:::{tip} Principe de superposition des amplitudes
Lorsqu'une transition peut se faire par deux ou
plusieurs alternatives en principe indiscernables, l'amplitude totale
est la somme des amplitudes associées à chaque voie.
:::
...
tel facteur est appelé **{index}`facteur de phase`**.
...
:::{tip} Principe d'addition des probabilités
Lorsqu'il existe plusieurs états finaux disjoints ou distincts la
probabilité de transition vers l'ensemble de ces états est la somme des
probabilités de transition vers chacun de ces états.
:::
...
d'addition des probabilités que nous avons déjà énoncée précédemment peut être
...
:::{tip} Principe de factorisation séquentielle
...
On considère une amplitude de probabilité :
$\alpha = 2 - i$

1. Calculer $\left| \alpha \right|^{2}$ ?

1. $\left| \alpha \right|^{2}$ peut-il être considérer comme une probabilité ?

1. Déterminer la constante de normalisation $N$ telle que $N \alpha$ soit une amplitude correctement normalisée donnant une probabilité admissible.
...
1. Interpréter $\alpha^{*} \beta $ dans le contexte d'un produit scalaire entre états quantiques ?

1. Déterminer le module et l'argument de $\alpha^{*} \beta$.

:::

:::{exercise}
:class: dropdown
:label: exo-Ap-composee

Une amplitude de probabilité est donnée par la somme des contributions :

$A = \left( 2 + i\right) \exp(i \phi) + \left( 1 - i\right)$

1. Développer $A$ en fonction de $\phi$.

1. Calculer  $\left| A \right|^{2}$ pour $\phi = 0$, $\phi = \pi/2$ et $\phi = \pi$ ?

1. Interpréter physiquement l'effet de phase $\phi$ sur la probabilté ?
...
Analysons cette expérience au moyen du PSAP.
 ...
 Il existe une infinité de chemins allant de la source $S$  au détecteur $D_2$ passant par
la fente.

A chacun de ces chemins, on associe une amplitude de probabilité $A$ dont la
phase $\Phi$ est déterminée par le chemin optique considéré selon {cite}`feynman1979` :
...
Pour un photon dans un milieu optique, on <u>admettra</u> que l'**{index}`action optique`** est donnée par :

$$S = \frac{E}{c}\int{n(r)\;ds}$$

où $E = \hbar \;\omega = \hbar \;c\;k$ est l'énergie du photon (voir {eq}`eq-PlanckEinstein` du chapitre [La lumière](#chap-lumiere)), $ds$ l'élément d'intégration curviligne et $n(r)$ l'indice de réfraction dépendant de la position donnée $r$ pour un milieu non homogène.
...
On a donc d'après {eq}` eq-APPhoton` :

$$A = exp\left(j\frac{S}{\hbar}\right) = \exp \left(j \;k \;n \;L\right)$$


Les différents chemins étant indiscernables, il faut appliquer le PSAP. Il faut donc
...
à la probabilité de collecter un photon.

```{note}
Les chemins ayant une longueur différentes, la phase accumulée $\Phi$ pour chaque chemin n'est pas la même. Il existe donc un déphasage entre les amplitudes associées à chaque chemin.
````

Si la somme des amplitudes est
nulle dans une direction donnée ($\theta$) de $D_2$, il est clair que $D_2$ ne
reçoit aucun photon.
...
Ainsi si la différence de phase entre les amplitudes associées aux chemins passant par les extrémités de la fente ne dépasse pas $\pi$ , la somme des
amplitudes ne peut pas être nulle.

Pour des raisons de simplification, on admettra que les deux chemins possibles représentés en rouge et en bleu sur la {numref}`fig-PSAPPhotons` sont composés de parties rectilignes. Si $d$ est la largeur de la fente, la différence de marche $\delta$ entre le chemin le
plus court (passant par l'extrémité inférieure de la fente {ref}`fig-PSAPPhotons`)  et le chemin le
plus long est :
...
En multipliant par la constante de Planck chaque membre de l'égalité
précédente {eq}`eq-thetaMin` on a :
...
```{important}
Les inégalités d'Heisenberg découle du PSAP.
```

```{note}
On verra dans le cours de L3 que les inégalités d'Heisenberg sont la conséquence de la {index}`dualité onde-impulsion` dans laquelle la description d'un état quantique en position et en impulsion correspond à des fonctions d'onde en position et en impulsion reliées par la transformée de Fourier.
```
...
<a href="https://mybinder.org/v2/gh/ngAtUbo/MQ_test_interactif/main?filepath=notebooks/testqutip.ipynb" target="_blank"> Test : ouvrir ce notebook dans Binder </a>
