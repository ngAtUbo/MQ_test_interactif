...
description: "Idées fondatrices de l'approche quantique."
authors:
...
Cependant nous observons que l'homogénéité de la direction des vitesses
($v_{y} = 0$ avant la fente) du faisceau d'électrons a été détruite du fait du passage des
électrons par la fente.
...
```{important}
$\alpha$ est donné par la loi empirique :

$$\alpha = \lambda/\delta y$$

à la condition **d'associer à l'électron une longueur
d'onde** $\lambda$ que l'expérience montre égale à :
...
Ce phénomène est appelé un phénomène de **{index}`diffraction`**  auquel la physique classique n'apporte aucune
réponse.
...
```{figure} media/A565_G_P_Thomson_Electron_Diffraction.jpg
:name: fig-diffelec
:alt: Expérience de Davisson et Germer
:width: 90%
Figure de diffraction d'électron pour une fente (Thomson )
```
...
En 192 De Broglie {cite}`de1929wave` a émis l'hypothèse que cette dualité est une
...
cristal à la manière de l'expérience de diffraction par un cristal de [Friedrich](wiki:Walter_Friedrich) et Knipping {cite}`friedrich1913phenomenes` des rayons X [^38].

En 1928, Davisson et Germer {cite}`davisson1928reflection` réalisèrent une expérience de diffraction
...
La relation de [Bragg](wiki:William_Lawrence_Bragg) démontrée dans le cas de la diffraction des rayons X est :
...
désignées de **{index}`quantons`** ou
...
On a donc une fonction $\cos\left( k_{0}x \right)$ qui oscille très rapidement comme le montre la figure tracée par le [script Python](#markdown-WP). L'amplitude est modulée par une
...
infinité d'images du lobe centrale comme le montre la figure ci-dessus. En réalité il est possible de construire une
...
Cette grandeur est appelée {index}`vitesse de groupe`. Elle correspond précisément à la vitesse de
...
On considère en exemple un [paquet d'onde gaussien](#markdown-WPG) où la sommation devient une intégrale. On montre que cette fois-ci la sommation intégrale d'ondes de nombres d'onde "infiniment rapprochés" conduit à une intérférence constructive dans une seule région localisée de l'espace :

$$
f(x,t)=\int_{-\infty}^{+\infty}
\exp\left[-\frac{a^2(k-k_0)^2}{4}\right]
e^{i(kx-\omega t)}\, dk
$$
...
le nom de [{index}`microscope de Heisenberg`](https://books.google.fr/books?hl=fr&lr=&id=T9zCAgAAQBAJ&oi=fnd&pg=PA1&dq=%40book%7Bheisenberg2013physical,+++title%3D%7BThe+physical+principles+of+the+quantum+theory%7D,+++author%3D%7BHeisenberg,+Werner%7D,+++year%3D%7B2013%7D,+++publisher%3D%7BCourier+Corporation%7D+%7D&ots=N0zf6WjzJV&sig=q1ivss74H0jf760rIm4Wil_tJxQ&redir_esc=y#v=onepage&q&f=false){cite}`heisenberg2013physical` . Nous allons la décrire ici.
...
perturbation provoquée par la mesure. On peut évaluer une telle perturbation en mesurant le changement
...
c'incertitude sur l'ordonnée de l'électron est de l'ordre de
$\Delta y = \delta$.
...
${p"}_{y}^{e} = \frac{h}{\lambda} - \frac{h}{\lambda"}\sin\frac{\theta}{2}$
et
${p'}_{y}^{e} = \frac{h}{\lambda} + \frac{h}{\lambda'}\sin\frac{\theta}{2}$
...
une formulation plus générale appelé {index}`inégalité d’Heisenberg` :
...
La première hypothèse suppose que la position et l'impulsion ont des valeurs bien définies dans la nature mais que les limitations liées au processus de mesure (**{index}`observer effect`**) ou aux appareils de mesure nous empêchent de
...
Ainsi les quantités $\Delta x$ et $\Delta p_{x}$ fournissent une approximation ou une estimation de $\left\langle|x| \right\rangle$ et de $\left\langle\left| p_{x} \right| \right\rangle$. Si on
...
1. Montrer que $\left\langle|x| \right\rangle = \sqrt{\frac{2}{\pi}}\sigma$. On utilisera la parité de l'intégrante pour réduire le domaine d'intégration. En outre, on fera le changement de variable $u = \frac{x^2}{2 \sigma^2}$ pour calculer l'intégrale.
1. Si $\Delta x = \sigma$, calculer $\left\langle|x| \right\rangle$ montratn ainsi que $\Delta x$ est une bonne estimation de $\left\langle|x| \right\rangle$.
:::
...
aussi bien pratique que théorique : l'{index}`effet tunnel`  dont la découverte est largement attribuée à {cite}`gamow1928quantentheorie`.
