...
```{note}
On verra dans le cours de L3 que les inégalités d'Heisenberg sont la conséquence de la {cite}`dualité onde-impulsion` dans laquelle la description d'un état quantique en position et en impulsion correspond à des fonctions d'onde en position et en impulsion reliées par la transformée de Fourier.
```
