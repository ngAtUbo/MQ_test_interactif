...
1. Calculer le polynôme caractéristique $mathcal{det}\left( \widehat{A} - \lambda \widehatI \right)$

1. En déduire les valeurs propres de $\widehat{A}$.

1. Pour chaque valeur propre $\lamda_i$, déterminer le vecteur propre associé par résolution de $\widehat{A} v_i =\lamda_i v_i$. On normalisera à l'unité chaque vecteur propre.
...
1. La décomposition spectrale de $\widehat{A}$ s'écrit : $\widehat{A} = \sum_{i=1}^{3} \lamda_i \left| v_{i} \right\rangle \left\langle v_{i} \middle|$. Montrer que l'action de cette décomposition sur chaque vecteur propre est conforme à la notion de vecteur propre.

1. Ecrire $\widehat{A} = \sum_{i=1}^{3} \lamda_i \left| v_{i} \right\rangle \left\langle v_{i} \middle|$ dans la base des états $\left\{ \left| e_{i} \right\rangle \right\}$. Conclusion.
:::
...
1. $mathcal{det}\left( \widehat{A} - \lambda \widehatI \right)=\left( 2-\lambda\right)\left(\lambda^2-5\lambda+4\right)=\left( 2-\lambda\right)\left(\lambda-1\right)\left(\lambda-4\right)$
