...
hbar = 1.055e-34
m = 9.11e-31  # kg, électron

# Électron ~150 eV -> k0 ~ 6.3e10 m^-1
k0 = 6.3e10  # m^-1

# Largeur en k : sigma_k ~ 3e9 m^-1 (~5% de k0)
sigma_k = 1.5e10
a = np.sqrt(2) / sigma_k  # ~ 5e-10 m

# Grille spatiale (en mètres)
x = np.linspace(-10e-8, 10e-8, 600)  # par ex. ±10 nm

# Vitesse de groupe
vg = hbar * k0 / m

# Temps pour parcourir ~10 nm
L = 1.7e-8
tL = L / vg

# Grille en k : ±5 sigma_k autour de k0
k_min = k0 - 5*sigma_k
k_max = k0 + 5*sigma_k
k = np.linspace(k_min, k_max, 4000)
...
for t in t_values:
    f_vals = np.array([f_of_x(xi, t, k, a, k0, hbar, m) for xi in x])
    prob_density = np.abs(f_vals)**2
    plt.plot(x, prob_density, label=f"t = {t:.2e} s")

plt.xlabel("x (m)")
plt.ylabel(r"|ψ(x,t)|²")
plt.title("Paquet d’onde gaussien (électron ~150 eV)")
plt.legend()
plt.grid(True)
plt.tight_layout()
plt.show()
...
le nom de {index}`microscope de Heisenberg`. Nous allons la décrire ici.
...
aussi bien pratique que théorique : l'{index}`effet tunnel`  dont la découverte est largement attribuée à Gamow {cite}`gamow1928quantentheorie`.
