...
1. Déterminer la constante de normalisation $N$ telle que $N \alpha$ soit une amplitude correctement normalisée donnat une probabilité admissible.
...
$A = \left( 2 + i\right) \exp{i v\phi} + \left( 1 - i\right)$

1. Développer $A$ en fonction de $\phi$.

1. Calculer  $\left| \alpha \right|^{2}a$ pour $\phi$ = 0$, $\phi$ = \pi/2$ et $\phi$ = \pi$ ?
