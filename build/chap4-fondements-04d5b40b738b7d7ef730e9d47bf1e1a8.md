...
k0 = 5.0     # nombre d’onde central

# Grille spatiale
x = np.linspace(-10, 10, 400)

# Grille en k pour l'intégration
k_min, k_max = k0 - 10, k0 + 10
k = np.linspace(k_min, k_max, 3000)

# Fonction f(x,0) = ∫ exp[-a^2 (k-k0)^2 / 4] * exp(i k x) dk
def f_of_x(x):
...
plt.title("Paquet d’onde gaussien à t = 0")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
aussi bien pratique que théorique : l\'effet tunnel {index}`Effet tunnel` dont la découverte est largement attribuée à Gamow {cite}`gamow1928quantentheorie`.
