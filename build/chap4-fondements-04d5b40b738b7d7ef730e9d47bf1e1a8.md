...
Cependant si on observe le faisceau d\'ÃƒÆ’Ã‚Â©lectrons qui a traversÃƒÆ’Ã‚Â© la
fente, nous mesurons que l\'homogÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©itÃƒÆ’Ã‚Â© de la direction des vitesses
($v_{y} = 0$ avant la fente) a ÃƒÆ’Ã‚Â©tÃƒÆ’Ã‚Â© dÃƒÆ’Ã‚Â©truite par le fait d\'obliger les
ÃƒÆ’Ã‚Â©lectrons ÃƒÆ’Ã‚Â  passer par la fente. Le faisceau issu de la fente a subi un
ÃƒÆ’Ã‚Â©talement de la distribution angulaire des trajectoires dans un angle
$\alpha$ **<u>dont la mesure expÃƒÆ’Ã‚Â©rimentale donne</u>**
$\lambda/\delta y$ ÃƒÆ’Ã‚Â  la condition d'associer ÃƒÆ’Ã‚Â  l'ÃƒÆ’Ã‚Â©lectron une longueur
d'onde</uÃƒÂ¢Ã…Â¸Ã‚Â©$\lambda$ que l\'expÃƒÆ’Ã‚Â©rience montre ÃƒÆ’Ã‚Â©gale ÃƒÆ’Ã‚Â  :
...
Ce phÃƒÆ’Ã‚Â©nomÃƒÆ’Ã‚Â¨ne est appelÃƒÆ’Ã‚Â© un phÃƒÆ’Ã‚Â©nomÃƒÆ’Ã‚Â¨ne de **diffraction** {index}`Diffraction` auquel la physique classique n\'apporte aucune
rÃƒÆ’Ã‚Â©ponse.
...
cristal ÃƒÆ’Ã‚Â  la maniÃƒÆ’Ã‚Â¨re de l\'expÃƒÆ’Ã‚Â©rience de diffraction des rayons X [^38]
...
La relation de Bragg dÃƒÆ’Ã‚Â©montrÃƒÆ’Ã‚Â©e dans le cas de la diffraction des rayons X est :
...
dÃƒÆ’Ã‚Â©signÃƒÆ’Ã‚Â©es de **quantons</u>** {index}`Quantons` ou
...
Cette grandeur est appelÃƒÆ’Ã‚Â©e vitesse de groupe {index}`Vitesse de groupe`. Elle correspond prÃƒÆ’Ã‚Â©cisÃƒÆ’Ã‚Â©ment ÃƒÆ’Ã‚Â  la vitesse de
...
On prend en exemple un paquet d'onde gaussien :

$$\psi(x,t) = \frac{ a**0.5}{(2*\pi)^{3/4}}\int_{-\infty}^{+\infty}{\exp{-\frac{ a**2*(k-k_0)}{4}{2*\pi}}*\exp{i*(kx-\omega*t)}*dk}$$
...
# ParamÃƒÆ’Ã‚Â¨tres du paquet dÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢onde
a = 1.0      # largeur en espace des ondes k
k0 = 5.0     # nombre dÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢onde central

# Grille spatiale
x = np.linspace(-10, 10, 400)

# Grille en k pour l'intÃƒÆ’Ã‚Â©gration
k_min, k_max = k0 - 10, k0 + 10
k = np.linspace(k_min, k_max, 3000)

# Fonction f(x,0) = ÃƒÂ¢Ã‹â€ Ã‚Â« exp[-a^2 (k-k0)^2 / 4] * exp(i k x) dk
def f_of_x(x):
...
# Calcul du paquet dÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢onde
f_vals = np.array([f_of_x(xi) for xi in x])

# Module au carrÃƒÆ’Ã‚Â©
prob_density = np.abs(f_vals)**2

# TracÃƒÆ’Ã‚Â©
plt.figure(figsize=(8,4))
plt.plot(x, prob_density, label=r"$|f(x,0)|^2$")
plt.xlabel("x")
plt.ylabel(r"$|f(x,0)|^2$")
plt.title("Paquet dÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢onde gaussien ÃƒÆ’Ã‚Â  t = 0")
plt.grid(True)
plt.legend()
plt.tight_layout()
plt.show()
...
le nom de microscope de Heisenberg {index}`Microscope de Heisenberg`. Nous allons la dÃƒÆ’Ã‚Â©crire ici.
...
une formulation plus gÃƒÆ’Ã‚Â©nÃƒÆ’Ã‚Â©rale appelÃƒÆ’Ã‚Â© inÃƒÆ’Ã‚Â©galitÃƒÆ’Ã‚Â© d'Heisenberg {index}`InÃƒÆ’Ã‚Â©galitÃƒÆ’Ã‚Â© dÃƒÂ¢Ã¢â€šÂ¬Ã¢â€žÂ¢Heisenberg` :
...
<u>La premiÃƒÆ’Ã‚Â¨re hypothÃƒÆ’Ã‚Â¨se</u> suppose que la position et
l\'impulsion ont des valeurs bien dÃƒÆ’Ã‚Â©finies dans la nature mais que les
limitations liÃƒÆ’Ã‚Â©es au processus de mesure (*observer effect* {index}`observer effect`) ou aux appareils de mesure nous empÃƒÆ’Ã‚Âªchent de
...
aussi bien pratique que thÃƒÆ’Ã‚Â©orique : l\'effet tunnel {index}`Effet tunnel` dont la dÃƒÆ’Ã‚Â©couverte est largement attribuÃƒÆ’Ã‚Â©e ÃƒÆ’Ã‚Â  Gamow {cite}`gamow1928quantentheorie`.
