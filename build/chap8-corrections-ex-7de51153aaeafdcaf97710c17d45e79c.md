...
1. $\left\langle|x| \right\rangle = \int_{-\infty}^{+-\infty}|x|\rho(x)dx = \frac{2}{\sigma\sqrt{2\pi}}\int_{0}^{+-\infty}x\exp \left(\frac{-x^{2}}{2\sigma^2}\right) dx$ \\soit (après changement de variable) :\\$\left\langle|x| \right\rangle = \sigma\sqrt{\frac{2}{\pi}}$
