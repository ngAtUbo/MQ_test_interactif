...
1. La décomposition spectrale de $\widehat{A}$ s'écrit : $\widehat{A} = \sum_{i=1}^{3} \lambda_i \left| v_{i} \right\rangle \left\langle v_{i} \middle|$. Montrer que l'action de cette décomposition sur chaque vecteur propre est conforme à la notion de vecteur propre.

1. Ecrire $\widehat{A} = \sum_{i=1}^{3} \lambda_i \left| v_{i} \right\rangle \left\langle v_{i} \middle|$ dans la base des états $\left\{ \left| e_{i} \right\rangle \right\}$. Conclusion.
:::
